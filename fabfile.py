...
def update_changed_requirements():
    ...
    get_reqs = lambda: run("cat %s" % reqs_path, show=False)  # noqa
...
def install():
    ...
    apt(
        "nginx libjpeg-dev python-dev python3-dev python-setuptools git-core "
...
def create():
    ...
    with project():
        if env.reqs_path:
            pip("-r %s/%s" % (env.proj_path, env.reqs_path))
        pip(
            "gunicorn setproctitle psycopg2-binary "
